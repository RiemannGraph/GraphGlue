...
def search_triangles(
    ...
    """
    Sample triangles (i, j, k) such that (i,j), (j,k), (k,i) all exist.

    :param edge_index: [2, E] undirected graph edges
    :param num_path_samples: number of triangles to sample per trial. If None, return all.
    :param path_sample_times: number of independent sampling trials.
    :param return_relabel_mapping: if True, also return mapping from relabeled index to original node index.

    :return:
        if return_relabel_mapping=False:
            -> Tensor [path_sample_times, 3, num_samples]
        else:
            -> (Tensor [path_sample_times, 3, num_samples], node_mapping, inverse_mapping)
                node_mapping: [num_unique_nodes] new index → original node index
                inverse_mapping: dict or LongTensor, original node index → new index
    """
    assert path_sample_times >= 1, "sampling times must be at least 1"
    ...
    paths = paths[paths[:, 0] != paths[:, 2]]  # remove i == k
    ...
    mask = torch.isin(query_hash, edge_hash)
    triangles = paths[mask]
    ...
    if num_path_samples is None:
        if not return_relabel_mapping:
            ...
            relabeled_triangles = inverse_flat.view_as(triangles).t().contiguous()
            ...
            return relabeled_triangles, unique_nodes, inv_map

    # ===== Step 4: sampling triangles =====
    node_degree = degree(edge_index[0], dtype=torch.float)
    ...
    if path_sample_times == 1:
        ...
        sampled_triangles = triangles[sampled_idx]  # [N, 3]
        sampled_triangles = sampled_triangles.t().contiguous().unsqueeze(0)  # [1, 3, N]
    else:
        sampled_triangles_list = []
        for _ in range(path_sample_times):
            sampled_idx = torch.multinomial(prob, num_path_samples, replacement=False)
            sampled_triangles_list.append(triangles[sampled_idx].t().contiguous())  # [3, N]
        sampled_triangles = torch.stack(sampled_triangles_list, dim=0)  # [T, 3, N]
    ...
    for t in range(path_sample_times):
        tri_flat = sampled_triangles[t].t()  # [N, 3]
        unique_nodes, inverse_flat = torch.unique(tri_flat, return_inverse=True)
        relabeled_tri = inverse_flat.view_as(tri_flat).t().contiguous()  # [3, N]
    ...
    relabeled_triangles = torch.stack(relabeled_list, dim=0)  # [T, 3, N]
